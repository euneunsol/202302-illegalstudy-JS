...
```JS
function checkGang(count, link, good) { // 콜백함수와 화살표 함수
  count < 3 ? link() : good();
}

const linkGang = () => console.log('세븐틴 사랑해');

const goodGang = () => console.log('세븐틴을 만날 확률이 높아집니다! :)');
...
```JS
function strReverse(str) { //문자열 뒤집기 
    if (str.length == 1) {
        return str 
    }
    return str[str.length-1] + strReverse(str.slice(0, str.length-1)); 
}
console.log(strReverse('euneunsol'));
```
